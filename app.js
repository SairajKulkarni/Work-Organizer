              ...
              if (foundUser) {
                ...
                app.post("/list", function (req, res) {
                  if (req.isAuthenticated()) {
                    ...
                    if (
                      ...
                      List.findOne(
                        { name: listName, user: foundUser._id },
                ...
                app.post("/search-task", function (req, res) {
                  if (req.isAuthenticated()) {
                    ...
                    Task.findOne(
                      { name: searchedTask, user: foundUser._id },
                      function (err, foundTask) {
                        if (!err) {
                          if (foundTask) {
                            console.log(foundTask);
                            List.findOne(
                              { _id: foundTask.listId, user: foundUser._id },
                        ...
                        function (err, foundedItem) {
                          if (!err) {
                            ...
                            List.findOneAndRemove(
                              { groupId: checkedItemId },
                              function (err, founded) {
                                if (!founded || err) {
                                  console.log(
                                    "No list associated with the checked item"
                                  );
                                } else {
                                  Task.find(
                                    { listId: founded._id },
                                    function (err, relatedTask) {
                                      if (err || !relatedTask) {
                                        console.log(
                                          "No list associated with the checked item"
                                        );
                                          ...
                                          element.deleteOne(
                                            ...
                                            function (err) {
                                              if (err) {
                                                ...
                                                console.log(
                                                  "Deleted the tasks in selected group"
                                                );
                                              }
                                            }
                                          );
                            ...
                            Task.deleteOne(
                              ...
                              function (err) {
                                if (err) {
                                  ...
                                  console.log(
                                    "Deleted the checked Item from Task collection also"
                                  );
                                  res.redirect("/" + listName);
                                }
                              }
                ...
                app.get("/:anything", function (req, res) {
                  if (req.isAuthenticated()) {
                    ...
                    List.findOne(
                      { name: customListName, user: foundUser._id },
                      function (err, foundList) {
                        if (!err) {
                          if (!foundList) {
                            // Create new list
